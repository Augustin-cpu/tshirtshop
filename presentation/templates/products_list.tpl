...
{if $obj->mSearchDescription != ""}
    <p class="lead">{$obj->mSearchDescription}</p>
{/if}
                ...
                <div class="mt-auto">
                    <div class="price-section d-flex justify-content-center mb-1">
                        {if $obj->mProducts[k].discounted_price != 0}
                            {* Prix barré *}
                            <span
                                class="text-muted text-decoration-line-through me-2 fs-6">{$obj->mProducts[k].price|string_format:"%.2f €"}</span>
                            {* Nouveau prix *}
                            <span
                                class="h5 text-primary fs-5">{$obj->mProducts[k].discounted_price|string_format:"%.2f €"}</span>
                        {else}
                            {* Prix régulier *}
